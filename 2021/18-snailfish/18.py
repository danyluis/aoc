...
def split(splittable):
	...
	if value % 2:
		a, b = int(value / 2 - 0.5), int(value / 2 + 0.5)
		repl = f'[{a},{b}]'
	else:
		repl = f'[{value // 2},{value // 2}]'

	return replace(s, start, end, repl)
...
def max_pair_sum_magnitude(lines):
	...
	for i in range(len(lines) - 1):
		for j in range(i + 1, len(lines)):
			...
			mij = sum_magnitude([lines[i], lines[j]])
			mji = sum_magnitude([lines[j], lines[i]])
			mx = max(mx, mij, mji)
	return mx
